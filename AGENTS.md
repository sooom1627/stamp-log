...
- **Work units**: Epic / Story / Task / Sub in `docs/backlog.md`. Story branch once → plan → Task branch → Sub 完了まで実装 → merge Task into Story → merge Story to `main`. **Sub (`ST-###`) is the implementation unit** (test → confirm). **Do not commit unless the user asks** (then one Sub = one commit). Follow `.cursor/rules/agile-workflow.mdc`.
